...
{
    public class DoctorListByNameQueryHandler : ResponseHandler, IRequestHandler<GetDoctorListByNameQuery, Response<List<GetDoctorListDTO>>>
    {
        ...
        public async Task<Response<List<GetDoctorListDTO>>> Handle(GetDoctorListByNameQuery request, CancellationToken cancellationToken)
        {
            ...
            if (doctors?.Any() != true)
            {
                ...
                return NotFound<List<GetDoctorListDTO>>($"No doctors found with Name: {request.FullName}");
            }
