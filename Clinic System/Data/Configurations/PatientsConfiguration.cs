namespace Clinic_System.Data.Configurations
{
    ...
    public class PatientsConfiguration : IEntityTypeConfiguration<Patients>
    {
        public void Configure(EntityTypeBuilder<Patients> builder)
        {
            ...
            // HasOne: Patient له ApplicationUser واحد
            // WithOne: ApplicationUser له Patient واحد
            // HasForeignKey: يحدد Foreign Key
            // OnDelete(DeleteBehavior.Restrict): منع حذف ApplicationUser إذا كان مرتبط بـ Patient

            // تسمية Foreign Key Column
            builder.Property(p => p.ApplicationUserId)
                .HasColumnName("ApplicationUserId");

            // Index على ApplicationUserId للأداء
